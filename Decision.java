...
public class Decision extends Node implements ActionListener
{
    ...
    public Decision(Player player,  ArrayList<String> moves)
    {
        ...
        choose_path = new JTextField("CHOOSE YOUR PATH"); //the field that player writes his decision
        ...
        choose_path.addMouseListener(new MouseAdapter(){
            public void mouseClicked(MouseEvent ae){
                choose_path.setText(""); //empty the textfield when clicked
            }
        });
        ...
        info = new JTextArea();
        info.append("POSSIBLE MOVES \n"); //calculate the possible moves of player
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        {
            if(possibleMoves.get(i).equals(command)) //if players command is a possible move, then do it
            {
                flag = true;
            }
        }
        if (!flag) //else error message
        {
            command = "";
